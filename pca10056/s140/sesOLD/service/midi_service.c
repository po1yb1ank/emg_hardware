 ...
 * To change this template file, choose ToolsA | Templates
...
static uint32_t data_io_char_add(ble_midi_service_t * p_midi_service, const ble_midi_service_init_t * p_midi_service_init)
{
    ...
    attr_char_value.init_len     = sizeof(uint16_t);//to 16
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = sizeof(uint16_t);//to 16
...
uint32_t ble_midi_data_io_value_update(ble_midi_service_t * p_midi_service, uint16_t data_io_value)//to16
{
    ...
    gatts_value.len     = sizeof(uint16_t);//to 16
