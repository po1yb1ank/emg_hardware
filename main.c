...
#define DEVICE_NAME "EMG-control"   /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME "Pribore" /**< Manufacturer. Will be passed to Device Information Service. */
...
#define MAX_CONN_INTERVAL MSEC_TO_UNITS(150, UNIT_1_25_MS) /**< Maximum acceptable connection interval (0.2 second). */
...
uint16_t rheo_global;
nrf_saadc_value_t rheoval;
...
void saadc_sampling_event_init(void) {
  ...
  /* setup m_timer for compare event every 400ms 100 fs */
...
void saadc_callback(nrf_drv_saadc_evt_t const *p_event) {
  if (p_event->type == NRF_DRV_SAADC_EVT_DONE) {
    ...
    APP_ERROR_CHECK(err_code);
    rheoval = p_event->data.done.p_buffer[0];

    /*for (int i = 0; i < SAMPLES_IN_BUFFER; i++) {
      NRF_LOG_INFO("%d", p_event->data.done.p_buffer[i]);
      rheoval += p_event->data.done.p_buffer[i];
    }*/
...
void saadc_init(void) //void
{
  ret_code_t err_code;
  nrf_saadc_channel_config_t channel_config =
      NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(NRF_SAADC_INPUT_AIN0);
  //nrf_saadc_channel_config_t bchannel_config = NRF_DRV_SAADC_DEFAULT_CHANNEL_CONFIG_SE(NRF_SAADC_INPUT_AIN1);
  err_code = nrf_drv_saadc_init(NULL, saadc_callback);
  APP_ERROR_CHECK(err_code);
  err_code = nrf_drv_saadc_channel_init(0, &channel_config);
  APP_ERROR_CHECK(err_code);
  //nrf_saadc_channel_limits_set(NRF_SAADC_INPUT_AIN0, 15, 4200);
...
int main(void) {
  ...
  complicated_value mpu_compl;
  while (1) {
    nrf_drv_saadc_sample();
    nrf_delay_ms(10);
    ble_midi_data_io_value_update(&m_midi_service, rheoval);
    if (start_accel_update_flag == true) {
      app_mpu_read_accel(&accel_values);
      nrf_delay_ms(5);
      app_mpu_read_gyro(&gyro_values);
      mpu_compl.x = gyro_values.x;
      mpu_compl.y = gyro_values.y;
      mpu_compl.z = gyro_values.z;
      mpu_compl.xx = accel_values.x;
      mpu_compl.yy = accel_values.y;
      mpu_compl.zz = accel_values.z;
      // Send MUP notification if in valid connection
      if ((m_mpu.conn_handle != BLE_CONN_HANDLE_INVALID) && (m_mpu.is_notification_enabled)) {
        ble_mpu_update_c(&m_mpu, &mpu_compl);
      }
    ...
    //rf_saadc_value_t battery_level;
    //nrf_gpio_pin_write(10,0);
    //nrf_drv_saadc_sample_convert(1, &battery_level);
    //batt = battery_level;
  }
}
